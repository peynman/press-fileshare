...
class Plupload
{
    ...
    {
        $path = storage_path('app/plupload');
    ...
    public function chunks($name, Closure $closure)
    {
        ...
        $filePath = $this->getChunkPath().'/'.$originalName.$chunk.'.part';

        $this->removeOldData($filePath);
        $this->appendData($filePath, $file);

        if ($chunk == $chunks - 1) {
            $this->mergeChunks($chunks, $originalName);
            $file = new UploadedFile($this->getChunkPath().'/'.$originalName, $originalName, 'blob', UPLOAD_ERR_OK, true);
            $result = $closure($file);
            @unlink($this->getChunkPath().'/'.$originalName);
            for ($chunk = 0; $chunk < $chunks; $chunk++) {
                $chunkPath = $this->getChunkPath().'/'.$originalName.$chunk.'.part';
                @unlink($chunkPath);
            }
            return $result;
        }
